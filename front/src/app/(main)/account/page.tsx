...
export default function AccountManagement() {
  ...
  const [instructorStatus, setInstructorStatus] = useState<any>(null);
  const [loadingInstructorStatus, setLoadingInstructorStatus] = useState(true);
  ...
  const fetchInstructorStatus = async () => {
    try {
      const response = await api.get('/api/instructor/request/status');
      setInstructorStatus(response.data);
    } catch (error) {
      console.error('Failed to fetch instructor status:', error);
    } finally {
      setLoadingInstructorStatus(false);
    }
  };
      ...
      <div className="min-h-screen bg-gray-900">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-lg text-white">
                    {user?.is_instructor ? 'Instructor Account' : 'Standard Account'}
                    {user?.is_admin && ' (Admin)'}
                  </p>
                </div>
              </div>
          ...
          >
            ...
            {loadingInstructorStatus ? (
              <p className="text-gray-400">Loading...</p>
            ) : user?.is_instructor ? (
              <div className="space-y-4">
                <div className="flex items-center gap-2">
                  <CheckCircleIcon className="h-5 w-5 text-green-400" />
                  <span className="text-green-400">You have instructor access</span>
                </div>
                ...
                <div className="bg-yellow-900/20 backdrop-blur-sm border border-yellow-600/30 rounded-lg p-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-yellow-400 font-medium">
                      Request Status: {instructorStatus.status.charAt(0).toUpperCase() + instructorStatus.status.slice(1)}
                    </span>
                  </div>
                  <p className="text-sm text-gray-300">
                    Submitted on {new Date(instructorStatus.submitted_at).toLocaleDateString()}
                  </p>
                  {instructorStatus.status === 'denied' && instructorStatus.denial_reason && (
                    <p className="text-sm text-red-400 mt-2">
                      Reason: {instructorStatus.denial_reason}
                    </p>
                  )}
                </div>
                {instructorStatus.status === 'denied' && (
                  <button
              ...
              <div className="space-y-4">
                <p className="text-gray-300">
                  Become an instructor to create classes, manage students, and assign logic puzzles.
                </p>
                <div className="bg-blue-900/20 backdrop-blur-sm border border-blue-600/30 rounded-lg p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-sm text-blue-400 font-medium mb-1">Benefits of Instructor Access:</p>
                      <ul className="text-sm text-gray-300 space-y-1">
                        <li>• Create and manage classes</li>
                        <li>• Assign puzzles and track student progress</li>
                        <li>• Access to gradebook and analytics</li>
                        <li>• Custom rule sets for your classes</li>
                      </ul>
                    </div>
                  </div>
                </div>
